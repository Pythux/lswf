...
from lswf.patcher.time import time_in_mn_since_last_save, insert_last_save_time
from lswf.patcher.filesys import (
...
def log(msg): tools.log.log(logging.info, msg)
...
def try_patch(relative_path):
    ...
    log('already {} patch exist'.format(len(li_patch)))
    ...
    if ram_file != file_stored:
        ...
        if len(patch) < len_ram_file and total_size_in_disk < len_ram_file * 3:
            ...
            log('delete_old_and_mv_new_to_src')
